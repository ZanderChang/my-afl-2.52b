...
void __sanitizer_cov_trace_strcmp(uint64_t *Arg1, uint64_t *Arg2, uint64_t StrSize)
{
  FILE* fp = fopen(getenv("RUNTIME_LOG_PATH"), "a");
  if (fp)
  {
    if (StrSize == 0)
	    fprintf(fp, "[STRCMP] %s %s\n", Arg1, Arg2);
    else
      fprintf(fp, "[STRCMP] %.*s %.*s %d\n", StrSize, Arg1, StrSize, Arg2, StrSize);
    fclose(fp);
  }
}
